...
**Division concernée :** initialement `color_lenses` ; toggle affiché pour **tout produit** avec interne SPH +0.00 **en stock** (v1.4.7).

| Mode | Comportement client |
|------|---------------------|
| **No power** (défaut si toggle visible) | Masque prescription SPH et « 2 puissances différentes ». Quantité seule. Résout l’interne **+0.00**. |
| **Power** | Affiche sélecteur SPH (+ quantité, option bi-œil si plusieurs enfants avec puissances). Exclut +0.00 de la cascade JS. |

**Toggle storefront (v1.4.7) :** visible seulement si `noPowerChild.inStock` dans `get_storefront_matrix()` ; sinon pas de toggle, mode Power / prescription directe.
...
### 2.33 Convert multi-couleurs (session 2026-09-22)

- **Besoin :** un parent Color Lenses (ex. Gray, Sterling Gray, Honey, Brown) doit créer **tous** les internes (couleurs × puissances) en **un seul** Convert, sans relancer le wizard par couleur.
- **UI wizard** : `render_identity_fields( …, $multi_color = true )` → select Couleur `multiple` ; aide « Internals = colors × power combinations ».
- **JS** : `collectIdentity()` envoie `color` en tableau ; `selectedColorCount()` + `refreshCount()` postent `color_count` pour le compteur live.
- **Génération** : `extract_generation_color_ids()` ; `build_children_from_ranges()` boucle couleurs × combos ; unicité déjà `|c:{colorId}` ; plafond = total multiplié.
- **Parent meta** : `_optic_identity_catalog.color` = **première** couleur (`normalize_identity_catalog`) ; chaque interne garde sa propre `catalog.color`.
- **Compteur** : `count_children_from_ranges( $division, $ranges, $color_count )`.

**Méthodes :** `WC_Optic_SKU::extract_generation_color_ids()`, `normalize_identity_catalog()`, `build_children_from_ranges()`, `count_children_from_ranges()` ; `WC_Optic_Converter::prepare_args()` / `preview()`.
...
### 2.35 Panier Color — enrichissement rendu (session 2026-09-23)

- **Cause :** `color_label` souvent absent du payload session ; page commande (order meta) n’avait pas de Color ; stub lazy sans couleurs → pas de `wc_optic_color` SSR.
- **Fix :** `enrich_payload_colors()` / `enrich_eye_color()` au rendu panier, session, résumé HTML, admin order ; meta order **Color** ; item_data panier **Color** dédiée ; no-power utilise `wc_optic_left_child` ; stub lazy charge couleurs via index catalogue.
...
### 2.20 Convert — preloader wizard (session 2026-09-12)

**Besoin :** à l’ouverture du modal, division + boutons apparaissaient avant la carte produit (latence AJAX).

**Comportement :** `setWizardLoading(true)` au début de `loadProduct()` → spinner WP (`.spinner.is-active`) + texte « Loading product… » ; classe `#wc-optic-wizard-modal.is-loading` masque `#wc-optic-wizard-body`, steps et barre ; Next/Back désactivés. À la fin (succès ou erreur) → `setWizardLoading(false)`.
...
| `admin/class-wc-optic-admin-convert.php` | Gabarits + Convert / Converted / **Specifics** ; DataTables footer ; **multi-select Colors** (v1.9.5) |
| `class-wc-optic-power-template.php` | Option `wc_optic_power_templates` (v3 : name + segments globaux) |
| `class-wc-optic-converter.php` | Simple → optic ; rebuild replace ; **append specifics** ; multi-color prepare/preview (v1.9.5) |
...
| `assets/js/admin-convert.js` | Wizard Convert / Rebuild / Specifics ; `getAjaxUrl()` → `window.ajaxurl` (v1.4.11) ; From/To/Step acceptent `0` ; **multi-color identity + color_count** (v1.9.5) |
...
### Convert multi-couleurs (v1.9.5)

- [ ] Wizard Convert (Color Lenses) : select Couleur multi ; choisir Gray + Sterling Gray + Honey + Brown
- [ ] Compteur d’internes = (combos puissances) × 4 couleurs
- [ ] Un Convert → internes pour les 4 couleurs × toutes les puissances (SKU / unicité distincts)
...
### Panier / checkout couleur (v1.9.6 + v1.9.7)

- [ ] Choisir Honey (ou autre) + No power → panier affiche **Color: Honey** (ligne meta + résumé)
- [ ] Même couleur + Power SPH → panier / checkout affichent la couleur
- [ ] Ligne déjà en panier avant fix : après refresh panier, Color apparaît (enrichissement session)
...
3. **CHANGELOG.md** / version plugin : synchroniser à chaque bump — courant **1.9.7**.
...
15. **Pastilles + panier couleur (1.9.0–1.9.7)** : Convert multi-color ; POST `wc_optic_color` ; enrichissement Color au rendu panier/commande depuis `child_id` si label manquant. Si Color absente : vérifier que les internes ont `catalog.color` > 0 (re-Convert multi-couleurs si besoin).
